...
public class CommandVariant extends NPCCommand {
	...
	public void execute(final Player player, final Living npc, final CommandContext args) throws CommandException {
		if (!npc.supports(Keys.HORSE_VARIANT)) throw new CommandException(Text.of(TextColors.RED, "You can only use this on horse NPC's."));
		if (super.getGame().getEventManager().post(new NPCEvent.Edit(super.getContainer(), player, npc))) {
			throw new CommandException(Text.of(TextColors.RED, "Could not edit NPC: Event was cancelled!"));
		}
		npc.offer(Keys.HORSE_VARIANT, args.<HorseVariant>getOne("variant").get());
	}
}