...
public class CommandZombie extends NPCCommand {
	...
	public void execute(final Player player, final Living npc, final CommandContext args) throws CommandException {
		if (!(npc instanceof Zombie)) throw new CommandException(Text.of(TextColors.RED, "You can only use this on zombie NPC's."));
		if (super.getGame().getEventManager().post(new NPCEvent.Edit(super.getContainer(), player, npc))) {
			throw new CommandException(Text.of(TextColors.RED, "Could not edit NPC: Event was cancelled!"));
		}
		npc.offer(Keys.ZOMBIE_TYPE, args.<ZombieType>getOne("type").get());

		player.sendMessage(TextUtils.getMessage("You successfully changed the selected NPC's zombie type."));
	}
}