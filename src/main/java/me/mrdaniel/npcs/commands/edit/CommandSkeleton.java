...
public class CommandSkeleton extends NPCCommand {
	...
	public void execute(final Player p, final NPCMenu menu, final CommandContext args) throws CommandException {
		if (!menu.getNPC().supports(Keys.SKELETON_TYPE)) throw new CommandException(Text.of(TextColors.RED, "You can only use this on skeleton NPC's."));
		if (super.getGame().getEventManager().post(new NPCEvent.Edit(super.getContainer(), p, menu.getNPC(), menu.getFile()))) {
			throw new CommandException(Text.of(TextColors.RED, "Could not edit NPC: Event was cancelled!"));
		}

		SkeletonType skeleton = args.<SkeletonType>getOne("skeleton").get();

		menu.getNPC().offer(Keys.SKELETON_TYPE, skeleton);
		menu.getFile().setSkeleton(skeleton);
		menu.getFile().save();
	}
}