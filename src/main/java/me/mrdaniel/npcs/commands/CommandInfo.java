	...
	public CommandInfo(@Nonnull final PaginationService service) {
		this.list = service.builder().title(Text.of("[ ", TextColors.RED, "NPCs", TextColors.RESET, " ]"))
				...
				.contents(Text.of(TextColors.GOLD, "    [ General Commands ]"),
						...
						Text.builder().append(Text.of(TextColors.AQUA, " /npc zombie <zombietype> - Zombies")).onClick(TextActions.suggestCommand("/npc zombie <type>")).build(),
						Text.builder().append(Text.of(TextColors.AQUA, " /npc skeleton <skeletontype> - Skeletons")).onClick(TextActions.suggestCommand("/npc skeleton <type>")).build(),
