...
public class MainPage extends Page {
	...
	public void updatePage(final Living npc, final NPCFile file) {
		...
		if (npc.supports(Keys.HORSE_VARIANT)) { lines[++c] = TextUtils.getOptionsText("Variant", "/npc variant <variant>", file.getHorseVariant().map(v -> TextUtils.capitalize(v.getName().toLowerCase())).orElse("None")); }
		if (npc.supports(Keys.ZOMBIE_TYPE)) { lines[++c] = TextUtils.getOptionsText("Zombie", "/npc zombie <zombie>", file.getZombie().map(v -> TextUtils.capitalize(v.getName().toLowerCase())).orElse("None")); }
		if (npc.supports(Keys.SKELETON_TYPE)) { lines[++c] = TextUtils.getOptionsText("Skeleton", "/npc skeleton <skeleton>", file.getSkeleton().map(v -> TextUtils.capitalize(v.getName().toLowerCase())).orElse("None")); }
	}
}